from flask import Flask, render_template, request, jsonify, redirect, url_for, send_from_directory, session, abort, render_template_string
...
def init_db():
    ...
    colunas_novas = [
        ('grau_academico', 'TEXT'), ('instituicao_ensino', 'TEXT'), 
        ('registro_validacao', 'TEXT'), ('gerar_qrcode', 'TEXT'),
        ('diploma_frente', 'TEXT'), ('diploma_verso', 'TEXT'),
        ('certificado', 'TEXT'), ('historico', 'TEXT'), ('outros_docs', 'TEXT'),
        ('edital_concurso', 'TEXT'), ('data_homologacao', 'TEXT'), 
        ('dados_nomeacao', 'TEXT'), ('data_posse', 'TEXT'), ('data_exercicio', 'TEXT'),
        ('esfera_concurso', 'TEXT'), ('local_esfera', 'TEXT'), ('faculdade_slug', 'TEXT')
    ]
...
def salvar_multiplos_arquivos(file_storage_list, antigos=''):
    ...
    if nomes_salvos: return '|'.join(nomes_salvos)
    return antigos
...
def checar_autenticacao():
    ...
    rotas_livres = [
        ...
        'download_file', 'visualizar_documento', 'conselho_oab', 'visualizar_qrcode', 'gerar_posse', 'gerar_exercicio', 'static'
    ]
...
def login():
    if request.method == 'POST':
        ...
        conn = get_db_connection()
        membro = conn.execute("SELECT * FROM equipe WHERE usuario = ? AND status_acesso = 'Ativo'", (usuario_digitado,)).fetchone()
        conn.close()
...
def solicitar_acesso():
    ...
    if request.method == 'POST':
        ...
        if existente:
            ...
            conn.execute('INSERT INTO equipe (nome, cargo, usuario, senha, status_acesso) VALUES (?, ?, ?, ?, ?)', 
...
def index():
    ...
    conn.close()
    return render_template('index.html', total=total_alunos, ativos=equipe_ativa, pendentes=equipe_pendente)
...
def cadastro():
    if request.method == 'POST':
        ...
        ''', (
            ...
            dados['tipo_curso'], dados['curso'], dados.get('grau_academico', ''), dados.get('instituicao_ensino', ''), 
            dados.get('data_inicio', ''), dados.get('data_conclusao', ''), dados.get('carga_horaria', ''), dados['matricula'], 
            dados.get('registro_validacao', ''), dados.get('gerar_qrcode', 'Não'), 
            diploma_frente, diploma_verso, certificado, historico, outros_docs,
            dados.get('edital_concurso', ''), dados.get('data_homologacao', ''), dados.get('dados_nomeacao', ''), 
            dados.get('data_posse', ''), dados.get('data_exercicio', ''), dados.get('esfera_concurso', 'Federal'), dados.get('local_esfera', ''),
            dados.get('orgao', ''), dados.get('numero_registro', ''), dados.get('uf_registro', ''),
...
def alterar():
    ...
    if request.method == 'POST':
        ...
        conn = get_db_connection()
        alunos = conn.execute('SELECT * FROM alunos WHERE nome LIKE ? OR cpf = ?', ('%' + termo + '%', termo)).fetchall()
        conn.close()
...
def informacoes(tipo):
    ...
    if request.method == 'POST':
        termo = request.form.get('termo', '')
        conn = get_db_connection()
        alunos = conn.execute(f'''
            SELECT * FROM alunos 
            WHERE (nome LIKE ? OR cpf = ?) AND ({filtro_sql})
        ''', ('%' + termo + '%', termo)).fetchall()
        conn.close()
    return render_template('informacoes.html', alunos=alunos, tipo=tipo, titulo=titulo)
...
def painel_aluno(id):
    ...
    aluno = conn.execute('SELECT * FROM alunos WHERE id = ?', (id,)).fetchone()
    conn.close()
    if not aluno: return "Aluno não encontrado.", 404
...
def portal_do_aluno_publico(matricula):
    conn = get_db_connection()
    aluno = conn.execute('SELECT * FROM alunos WHERE matricula = ?', (matricula,)).fetchone()
    conn.close()
    if not aluno: return "Matrícula não encontrada. Verifique o link.", 404
    
    faculdade_slug = aluno['faculdade_slug'] if aluno['faculdade_slug'] else 'puc_go'
    
    logado_portal = False
    erro = None

    if request.method == 'POST':
        senha_digitada = ''.join(filter(str.isdigit, request.form.get('senha', '')))
    ...
    try:
        return render_template(f"portais/portal_{faculdade_slug}.html", aluno=aluno, url_base=request.host_url, logado_portal=logado_portal, erro=erro)
    except Exception as e:
        try:
            return render_template(f"portais/{faculdade_slug}.html", aluno=aluno, url_base=request.host_url, logado_portal=logado_portal, erro=erro)
        except:
            return f"O sistema tentou abrir o portal correspondente a '{faculdade_slug}', mas o arquivo do template não foi encontrado na pasta 'portais'.", 404
...
def validacao_qr_code(faculdade_slug, cpf):
    ...
    conn.close()
    if not aluno: 
        return "Aluno não encontrado. Verifique se o CPF existe no banco de dados.", 404
    
    slug = aluno['faculdade_slug'] if aluno['faculdade_slug'] else faculdade_slug

    try: 
        return render_template(f"portais/portal_{slug}.html", aluno=aluno, url_base=request.host_url, logado_portal=False, erro=None)
    except Exception as e: 
        try:
            return render_template(f"portais/{slug}.html", aluno=aluno, url_base=request.host_url, logado_portal=False, erro=None)
        except:
            return f"Arquivo de portal não encontrado na pasta templates/portais.", 404
...
def consulta_xml():
    ...
    if request.method == 'POST':
        ...
        if arquivo and arquivo.filename:
            ...
            ''', (busca_completa, busca_completa, busca_completa, busca_completa, 
                  busca_base, busca_base, busca_base, busca_base)).fetchone()
            conn.close()
...
def consulta_xml_direta(matricula):
    ...
    aluno = conn.execute('SELECT * FROM alunos WHERE matricula = ?', (matricula,)).fetchone()
    conn.close()
    if not aluno: return "Matrícula não encontrada.", 404
...
def imprensanacional_consulta(matricula):
    ...
    aluno = conn.execute('SELECT * FROM alunos WHERE matricula = ?', (matricula,)).fetchone()
    conn.close()
    if not aluno: return "Candidato não encontrado.", 404
    return render_template('consulta_imprensanacional.html', aluno=aluno, url_base=DOMINIOS_MAPA['dou'] + '/')

@app.route('/imprensanacional/busca')
def imprensanacional_busca():
    termo = request.args.get('q', '')
    return f"<div style='font-family: Arial; padding: 50px;'><h2>Pesquisa DOU: {termo}</h2></div>"
...
def visualizar_documento(aluno_id, tipo_doc):
    ...
    filenames_raw = ""
    titulo_doc = ""
    if tipo_doc == 'certificado': filenames_raw, titulo_doc = aluno['certificado'], "Certificado de Conclusão"
    elif tipo_doc == 'historico': filenames_raw, titulo_doc = aluno['historico'], "Histórico Escolar"
    elif tipo_doc == 'outros': filenames_raw, titulo_doc = aluno['outros_docs'], "Outros"
    else: return abort(400)
    filenames = [secure_filename(f) for f in (filenames_raw or "").split('|') if f]
    return render_template('visualizar_combinado.html', aluno=aluno, filenames=filenames, titulo_doc=titulo_doc)
...
def gerar_posse(id):
    ...
    conn.close()
    if not aluno: return "Candidato não encontrado.", 404
    return render_template('termo_posse.html', aluno=aluno, p_num=random.randint(10, 999), p_ano=random.randint(2023, 2026))
